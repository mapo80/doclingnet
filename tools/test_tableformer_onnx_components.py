#!/usr/bin/env python3
"""
Test TableFormer ONNX Components

This script implements the full autoregressive loop in Python using the 4 ONNX components:
1. Encoder
...
Usage:
    python test_tableformer_onnx_components.py --image path/to/image.png --visualize
"""
...
class TableFormerONNXInference:
    """TableFormer inference using ONNX components with autoregressive loop in Python."""
    ...
    def __init__(self, model_dir):
        ...
        print(f"   Special tokens: <start>={self.start_token}, <end>={self.end_token}, <pad>={self.pad_token}")
    ...
    def preprocess_image(self, image_path):
        ...
        Returns:
            image_tensor: (1, 3, 448, 448) preprocessed image
    ...
    def predict(self, image_path, max_steps=500):
        """
        Run full TableFormer prediction with autoregressive loop.
        ...
        Returns:
            dict with keys:
                ...
                - bbox_classes: numpy array (N, 3) bbox class predictions
                - bbox_coords: numpy array (N, 4) bbox coordinates [x1, y1, x2, y2]
        ...
        print(f"\n{'='*70}")
        print(f"INFERENCE ON: {Path(image_path).name}")
        print(f"{'='*70}")
        ...
        print(f"\n[1/4] Encoder: {encoder_time:.3f}s")
        print(f"      Output shape: {encoder_out.shape}")

        # Step 2: Tag Transformer Encoder
        tag_enc_start = time.time()
        memory = self.tag_encoder_session.run(None, {'encoder_out': encoder_out})[0]
        tag_enc_time = time.time() - tag_enc_start
        print(f"[2/4] Tag Transformer Encoder: {tag_enc_time:.3f}s")
        print(f"      Memory shape: {memory.shape}")

        # Step 3: Autoregressive Tag Decoding
        ...
        while step < max_steps:
            # Prepare input: all decoded tags so far (seq_len, batch_size)
            decoded_input = np.array(decoded_tags_ids, dtype=np.int64).reshape(-1, 1)

            # Run decoder step
            logits, tag_hidden = self.tag_decoder_session.run(
                None,
                {
                    'memory': memory,
            ...
            new_tag_str = self.inv_word_map.get(new_tag_id, f"<unknown:{new_tag_id}>")
        ...
        print(f"[3/4] Tag Decoding: {decode_time:.3f}s")
        print(f"      Steps: {step}")
        print(f"      Tags: {len(decoded_tags_str)}")
        ...
        if len(tag_hidden_states) > 0:
            ...
            tag_hidden_batch = np.stack(tag_hidden_states, axis=0)
        ...
        print(f"[4/4] BBox Decoding: {bbox_time:.3f}s")
        print(f"      BBoxes: {len(bbox_coords_list)}")
    ...
    def visualize(self, result, output_path):
        ...
        # Scale factor for visualization (2x for better visibility)
        scale = 2
        img_vis = cv2.resize(img, (img_w * scale, img_h * scale))

        # Draw bboxes
        for i, bbox in enumerate(result['bbox_coords']):
            # BBox coords are in normalized [0, 1] format relative to 448x448
...
def main():
    parser = argparse.ArgumentParser(description="Test TableFormer ONNX Components")
    parser.add_argument(
        "--model-dir",
        type=str,
    ...
    parser.add_argument(
        "--output",
        type=str,
        help="Output path for visualization (default: same dir as input with _onnx suffix)"
    )
    parser.add_argument(
        "--max-steps",
        type=int,
    ...
    inference = TableFormerONNXInference(args.model_dir)
    ...
    print("RESULTS")
    print(f"{'='*70}")
    print(f"Tags predicted: {len(result['tags'])}")
    print(f"BBoxes predicted: {len(result['bbox_coords'])}")
    print(f"Inference time: {result['inference_time']:.3f}s")
    print(f"\nTag sequence: {' '.join(result['tags'][:50])}")
    if len(result['tags']) > 50:
        print(f"... ({len(result['tags']) - 50} more)")
    ...
    if args.visualize:
        if args.output:
            ...
            output_path = input_path.parent / f"{input_path.stem}_onnx_components.png"
